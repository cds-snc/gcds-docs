...
const serveLocalFontFile = (route) => {
    var _a;
    ...
    if (fs.existsSync(localPath)) {
        return route.fulfill({
            contentType: (_a = FONT_CONTENT_TYPES[path.extname(fileName)]) !== null && _a !== void 0 ? _a : 'application/octet-stream',
...
export const test = base.extend({
    page: async ({ page }, use, testInfo) => {
        ...
        await page.waitForFunction(() => {
            const gcdsEls = Array.from(document.querySelectorAll('.preview-component *')).filter(el => el.tagName.toLowerCase().startsWith('gcds-'));
            return (gcdsEls.length > 0 &&
        ...
        // of the icon glyph.
        await Promise.race([
            page.evaluate(() => document.fonts.load('1em "gcds-icons"').then(() => document.fonts.ready)),
            page.waitForTimeout(10000),
        ]);
        await use(page);
    },
