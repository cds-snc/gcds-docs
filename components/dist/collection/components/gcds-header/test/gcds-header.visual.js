...
test.describe('gcds-header - desktop', () => {
    ...
    tests.map(name => {
        test(`${name}`, async ({ page }) => {
            ...
            await expect(page.locator(`[data-variant="${name}"] .preview-component`)).toHaveScreenshot(`desktop-${name}.png`);
        });
    });
});
test.describe('gcds-header - tablet', () => {
    ...
    tests.map(name => {
        test(`${name}`, async ({ page }) => {
            // Screenshot just the component, not the surrounding preview chrome
            await expect(page.locator(`[data-variant="${name}"] .preview-component`)).toHaveScreenshot(`tablet-${name}.png`);
        });
    });
});
test.describe('gcds-header - mobile', () => {
    ...
    tests.map(name => {
        test(`${name}`, async ({ page }) => {
            // Screenshot just the component, not the surrounding preview chrome
            await expect(page.locator(`[data-variant="${name}"] .preview-component`)).toHaveScreenshot(`mobile-${name}.png`);
        });
    });
});
