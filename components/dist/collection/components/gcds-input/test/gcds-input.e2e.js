...
describe('gcds-input', () => {
    it('renders', async () => {
        ...
        await page.setContent('<gcds-input label="Label" input-id="input-renders" name="input-renders-name"/>');
...
describe('gcds-input a11y tests', () => {
    ...
    it('aria-invalid', async () => {
        const page = await newE2EPage();
        await page.setContent('<gcds-input label="Label" input-id="aria-invalid" name="aria-invalid-name" error-message="Field required" />');
        const element = await await page.find('gcds-input >>> input');
    ...
    `);
        ...
        await page.keyboard.press('Tab');
        expect(await page.evaluate(() => window.document.activeElement.shadowRoot.activeElement.textContent)).toEqual(inputField);
    });
    /**
     * Input label test
     */
    it('input contains label', async () => {
        const page = await newE2EPage();
        await page.setContent('<gcds-input label="Label" input-id="contains-label" name="contains-label-name" />');
    ...
    it('input has aria-labelledby for label', async () => {
        const page = await newE2EPage();
        await page.setContent('<gcds-input label="Label" input-id="aria-labelledby" name="aria-labelledby-name" />');
        const element = await await page.find('gcds-input >>> input');
        expect(element.getAttribute('aria-labelledby')).toEqual('label-for-aria-labelledby');
    });
});
//# sourceMappingURL=gcds-input.e2e.js.map
