...
pageTitle: Justify self
description: 'The justify self class sets the justify-self property. It controls how an individual grid item is aligned along the inline (row) axis within its grid area, allowing it to override the container’s justify-items setting for that specific item.'
...
<Notice
  ...
  role="info"
  html={`In flexbox, <code>justify-self</code> is ignored, so it has no effect.`}
/>
