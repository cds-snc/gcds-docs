...
import anatomy from './assets/gcds-signature-anatomy-fr.svg';
import sideBySide from './assets/example-signature-side-by-side-fr.svg';
import sideBySideReversed from './assets/example-signature-side-by-side-reversed-fr.svg';
import blackWhite from './assets/example-signature-bw-fr.svg';
import singleColour from './assets/example-signature-single-colour-style-fr.svg';

## Structure de la signature

<AnatomyList>
  <li>La <strong>signature du gouvernement du Canada</strong> est l'identificateur de l'image de marque placée dans l'<gcds-link href={links.fr.header}>en-tête</gcds-link> du site. La signature sert de lien général vers la page d'accueil du site.</li>
  <li>Le <strong>mot-symbole «&nbsp;Canada&nbsp;»</strong> est l'identificateur de l'image de marque placé dans le <gcds-link href={links.fr.footer}>pied de page</gcds-link> du site. Il renforce l'image de marque en informant les visiteur·rice·s du site que le contenu qui leur est présenté provient du gouvernement du Canada.</li>
</AnatomyList>

<AnatomyImage
  src={anatomy.src}
  alt="Signature avec les étiquettes « Signature du gouvernement du Canada » et « Mot-symbole Canada ». Chaque élément du composant est identifié à l’aide d’un chiffre.]"
/>
...
<figure class="mb-600">
  <img class="b-sm b-default p-300 mb-100" src={sideBySide.src} alt="Une image représentant les deux versions de la signature. La signature se trouve à gauche et le mot-symbole est placé à droite." />
  <figcaption>Légende : Le style de couleur standard utilise un texte noir et un drapeau rouge sur fond blanc.</figcaption>
</figure>

<figure class="mb-600">
  <img class="b-sm b-default p-300 mb-100" src={sideBySideReversed.src} alt="Une image représentant les deux versions de la signature. La signature se trouve à gauche et le mot-symbole est placé à droite. Cette version utilise du texte blanc sur un fond noir." />
  <figcaption>Légende : Le style de couleur inversé utilise un texte blanc et un drapeau rouge sur fond noir.</figcaption>
</figure>

<figure class="mb-600">
  <img class="b-sm b-default p-300 mb-100" src={blackWhite.src} alt="Une image présentant deux combinaisons de signature et de mot-symbole. Dans un cas, la signature et le mot-symbole sont noirs sur fond blanc. Dans l'autre cas, la signature et le mot-symbole sont blancs sur fond noir." />
...
<figure class="mb-600">
  <img class="b-sm b-default p-300 mb-100" src={singleColour.src} alt="Une image montrant la signature et le mot-symbole en violet foncé sur un fond violet pâle. Des barres et des cases simulent du texte et des images sur une page web fictive. Les cases de texte et d'images sont également violet foncé." />
