...
const displayTokens = (token, subCategory, locale) => {
    ...
    case "color":
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        tableBody += `<tr>
          ...
          chroma.contrast(tokens[i].value, "#FFF") > 3 ? "white" : "black"
        }"
              >
              </div>
            </td>
            ...
            <td class="font-family-monospace">${tokens[i].value}</td>
      ...
      for (var i = 0; i < tokens.length; i++) {
        if (
          ...
          tableBody += `<tr>
            ...
            i
          ].value.lineHeight.replace("%", "")}% ${tokens[i].value.fontFamily};'
              >
  ...
  return `<table class="table-display-token-${token}">
    ${tableHeading}
    ${tableBody}
  </table>`;
};
